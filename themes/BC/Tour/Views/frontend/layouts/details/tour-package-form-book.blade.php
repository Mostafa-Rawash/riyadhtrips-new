                ...
                <div class="form-content">
                        ...
                        <div class="form-group form-guest-search" v-for="(type,index) in person_types">
                            <div class="guest-wrapper d-flex justify-content-between align-items-center">
                                <div class="flex-grow-1">
                                    ...
                                    <div class="render check-in-render">@{{type.display_price}} {{__("per person")}}
                                    </div>
                                </div>
                        ...
                        <div class="form-group" v-for="(type, index) in packages" :key="index">
                            <div class="extra-price-wrap d-flex justify-content-between">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <div class="render check-in-render" v-if="type.description">@{{ type.description }}
                                    </div>
                                ...
                                <div class="flex-shrink-0">
                                    <div v-if="type.start_times && type.start_times.length">
                                        <div v-for="(time, timeIndex) in type.start_times" :key="timeIndex">
                                            <label>
                                                <input type="radio" :name="'start_time_' + index" :value="time"
                    ...
                    @if(isset($row->meta->enable_time_slots) && $row->meta->enable_time_slots)
                        <div class="form-section-group form-group" v-if="enable_time_slots && start_date">
                            ...
                            <div v-else-if="available_time_slots && available_time_slots.length"
                                ...
                                <div class="time-slots-grid row">
                                    <div v-for="(slot, index) in available_time_slots" :key="index" class="mb-3">
                                        <div class="time-slot-card" :class="{
                                            ...
                                            :title="getSlotTooltip(slot)" role="button"
                                            ...
                                            <div class="time-slot-header">
                                                <div class="time-display">
                                                    ...
                                                    <span v-if="slot.end_time" class="end-time">- @{{
                                                        formatTime(slot.end_time) }}</span>
                                                </div>
                                                <div class="selection-indicator"
                                                    v-if="selected_time_slot && selected_time_slot.id === slot.id">
                                            ...
                                            <div class="time-slot-info">
                                                ...
                                                <div
                                                    class="capacity-info d-flex justify-content-between align-items-center">
                                                    <div class="capacity-text">
                                                        <span class="available-spots" :class="{
                                                    ...
                                                    <div class="price-modifier"
                                                        v-if="slot.price_modifier && slot.price_modifier !== 0">
                                                        <span class="modifier-amount"
                                                            :class="slot.price_modifier > 0 ? 'text-success' : 'text-danger'">
                                                            @{{ slot.price_modifier > 0 ? '+' : '' }}@{{
                                                            formatMoney(slot.price_modifier) }}
                                                        </span>
                                                    </div>
                                                </div>
                                            ...
                                            <!-- Disabled Overlay -->
                                            <div v-if="!isSlotBookableForCurrentGuests(slot) || slot.is_sold_out"
                                                class="disabled-overlay">
                                                <div class="disabled-content">
                                                    <i class="fa fa-ban"
                                                        v-if="!isSlotBookableForCurrentGuests(slot) && !slot.is_sold_out"></i>
                                ...
                                <div v-if="sold_out_slots && sold_out_slots.length" class="sold-out-toggle mt-3">
                                    <button type="button" class="btn btn-link btn-sm"
                                        @click="show_sold_out_slots = !show_sold_out_slots">
                                ...
                                <!-- Sold Out Slots -->
                                <div v-if="show_sold_out_slots && sold_out_slots && sold_out_slots.length"
                                    class="sold-out-slots mt-3">
                                    <h6 class="text-muted mb-2">{{__("Unavailable Times")}}</h6>
                                    <div class="row">
                                        <div v-for="(slot, index) in sold_out_slots" :key="'sold-' + index"
                            ...
                            <div v-else-if="!loading_time_slots && enable_time_slots"
                                class="no-time-slots text-center py-4">
                                <div class="mb-3">
                                    <i class="fa fa-calendar-times fa-3x text-muted"></i>
                                </div>
                                <h6 class="text-muted">{{__("No available time slots")}}</h6>
                                <p class="text-muted small mb-0">
                                    {{__("Please select a different date or contact us for availability.")}}
                                </p>
                            </div>
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <span class="ml-2">
                                            ...
                                            <span v-if="selected_time_slot.end_time">- @{{
                                                formatTime(selected_time_slot.end_time) }}</span>
                                        </span>
                                    </div>
                    ...
                    <div class="form-section-group form-group" v-if="discount_by_people_output.length">
                        ...
                        <div class="extra-price-wrap d-flex justify-content-between"
                            v-for="(type,index) in discount_by_people_output">
                            <div class="flex-grow-1">
            ...
            <div class="form-send-enquiry" v-show="enquiry_type=='enquiry'">
                ...
                <div class="enquiry-form">
                    <div class="enquiry-form-header">
                        ...
                        <h5 class="enquiry-form-description">
                            {{__("For a truly unique experience, contact one of our travel advisors via WhatsApp or enter your details and we will contact you to help customise your package!")}}
                        </h5>
                        <button type="button" class="btn btn-primary whatsapp-button-enquiry">
                            <a href="#" onclick="buildWhatsAppLink(event)" target="_blank" rel="noreferrer">
                                <span>
                                    <svg focusable="false" color="inherit" fill="currentcolor" aria-hidden="true"
                                        role="presentation" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet"
                                        width="24px" height="24px" class="icon-whatsapp">
                                        <g>
                                            <path
                                    ...
                                    </svg>{{__("Contact us via WhatsApp")}}</span>
                            </a>
                        </button>
                    </div>
                    ...
                    <div class="enquiry-form-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="enquiry_name"
                                value="{{$user->display_name ?? ''}}" name="enquiry_name"
                                placeholder="{{ __("Name *") }}">
                        </div>
                        <div class="form-group">
                            <label for="enquiry_email">{{ __("Email *") }}</label>
                            <input type="text" class="form-control" id="enquiry_email" value="{{$user->email ?? ''}}"
                                name="enquiry_email" placeholder="{{ __("Email *") }}">
                        </div>
                        <div class="form-group">
                            <label for="enquiry_phone">{{ __("Phone") }}</label>
                            <input type="text" class="form-control" id="enquiry_phone" value="{{$user->phone ?? ''}}"
                                name="enquiry_phone" placeholder="{{ __("Phone") }}">
                        </div>
                        <div class="form-group">
                            <label for="enquiry_note">{{ __("Note") }}</label>
                            <textarea class="form-control" id="enquiry_note" placeholder="{{ __("Note") }}"
...
{{-- Move scripts outside Vue component to avoid template compilation errors --}}
<script>
    // WhatsApp link builder function
    function buildWhatsAppLink(e) {
        ...
        const phone = document.getElementById('enquiry_phone')?.value || '';
        const note = document.getElementById('enquiry_note')?.value || '';

        let whatsappText = `Hi there! I'm interested in knowing more about {{$row->title}} - Tour id : {{$row->id}}`;

        if (name) whatsappText += `\n\nName: ${name}`;
        if (email) whatsappText += `\nEmail: ${email}`;
        if (phone) whatsappText += `\nPhone: ${phone}`;
        if (note) whatsappText += `\n\nNote: ${note}`;

        const encodedText = encodeURIComponent(whatsappText);
        const whatsappLink = `https://wa.me/966920032086?text=${encodedText}`;
...
<style>
    ...
    .enquiry-form-body>*:not(:nth-last-child(-n+2)) {
        margin-bottom: 30px;
    }

    .enquiry-form-body>*:nth-last-child(-n+1) {
        margin-bottom: 30px;
    }
