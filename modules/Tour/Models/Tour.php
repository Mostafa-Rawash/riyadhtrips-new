...

{
...

    {
        ...
        // Store time slot information if provided
        if ($request->input('time_slot_id')) {
            $booking->time_slot_id = $request->input('time_slot_id');
        ...
        if ($check) {
            ...
            // Store time slot information in meta
            if ($request->input('time_slot_id')) {
                $timeSlot = $this->tourTimeSlotClass::find($request->input('time_slot_id'));
                if ($timeSlot) {
                    $booking->addMeta('time_slot_info', [
                        'time_slot_id' => $timeSlot->id,
                        'start_time' => $timeSlot->start_time,
                        'end_time' => $timeSlot->end_time,
                        'day_of_week' => $timeSlot->day_of_week,
                        'formatted_time' => $timeSlot->formatted_time ?? $timeSlot->start_time
                    ]);
                }
            }
    ...
    public function beforeCheckout(Request $request, $booking)
    {
        ...
        if ($booking->time_slot_id) {
            ...
            if (!$timeSlot) {
                return $this->sendError(__("Selected time slot is no longer available"));
            }
            
            if (!$timeSlot->active) {
                return $this->sendError(__("Selected time slot has been deactivated"));
            }
            ...
            if ($booking->total_guests > $remainingCapacity) {
                return $this->sendError(__("This time slot only has :available spots available, but your booking requires :requested spots. Please select a different time or reduce the number of guests.", [
                    'available' => $remainingCapacity,
                    'requested' => $booking->total_guests
                ]));
            }
            
            // Check booking cutoff
            if ($timeSlot->isBookingCutoffReached($bookingDate)) {
                return $this->sendError(__("Booking deadline has passed for this time slot"));
            }
            
        } else {
            // 🔥 CRITICAL: For regular tours without time slots
            $maxGuests = $this->getNumberAvailableBooking($booking->start_date);
            if ($booking->total_guests > $maxGuests) {
                return $this->sendError(__("There are only :maxGuests spots available for your selected date, but you need :requested spots. Please reduce the number of guests or select a different date.", [
                    'maxGuests' => $maxGuests,
                    'requested' => $booking->total_guests
                ]));
            }
...

    {
        ...
        // 🔥 CRITICAL: Calculate total guests properly
        if (!empty($request->person_types)) {

            $totalGuests = array_sum(Arr::pluck($request->person_types, 'number')) ?? 0;

        } else {
            $totalGuests = (int) $request->guests;
        }

        // 🔥 CRITICAL: Validate minimum guests
        if ($totalGuests < 1) {
            return $this->sendError(__("Please select at least 1 guest"));
        }

        // 🔥 CRITICAL: Enhanced Time Slot Validation with Real-time Capacity Check
        if ($request->input('time_slot_id')) {
            $timeSlotId = $request->input('time_slot_id');
            $timeSlot = $this->tourTimeSlotClass::find($timeSlotId);
            
            if (!$timeSlot) {
                return $this->sendError(__("Selected time slot is not valid"));
            }
            
            if (!$timeSlot->active) {
                return $this->sendError(__("Selected time slot is not available"));
            }
            
            // Check if time slot is for the correct day
            ...
            if ($totalGuests > $remainingCapacity) {
                return $this->sendError(__("This time slot only has :available spots available, but you need :requested spots. Please select a different time or reduce the number of guests.", [
                    'available' => $remainingCapacity,
                    'requested' => $totalGuests
                ]));
            }
            
            // Check if guests exceed time slot maximum capacity
            if ($totalGuests > $timeSlot->max_guests) {
                return $this->sendError(__("This time slot can accommodate maximum :max guests, but you selected :requested guests", [
                    'max' => $timeSlot->max_guests,
                    'requested' => $totalGuests
                ]));
            }
            
            // Check booking cutoff
            if ($timeSlot->isBookingCutoffReached($start_date)) {
                return $this->sendError(__("Booking deadline has passed for this time slot"));
            }
            
        } else {
            // 🔥 CRITICAL: For regular tours without time slots - Enhanced validation
            $numberGuestsCanBook = $this->getNumberAvailableBooking($start_date);
            
            if ($totalGuests > $numberGuestsCanBook) {
                return $this->sendError(__("There are only :available spots available for your selected date, but you need :requested spots. Please reduce the number of guests or select a different date.", [
                    'available' => $numberGuestsCanBook,
                    'requested' => $totalGuests
                ]));
            }
            
            // Check against tour maximum capacity
            if ($totalGuests > $this->max_people) {
                return $this->sendError(__("This tour can accommodate maximum :max guests, but you selected :requested guests", [
                    'max' => $this->max_people,
                    'requested' => $totalGuests
                ]));
            }
...

    {
        ...
        $booking_data = [
            ...
            // Time Slots
            'enable_time_slots'        => $meta->enable_time_slots ?? false,
